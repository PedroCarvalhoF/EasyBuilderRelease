...
window.easyLoginBase.download = {
    base64: function (fileName, contentType, base64) {
        try {
            ...
            const blob = new Blob([uInt8Array], { type: contentType || "application/octet-stream" });
            ...
            console.warn("Input is not valid base64, downloading as raw text instead...", ex);
            const blob = new Blob([base64 || ""], { type: contentType || "text/plain;charset=utf-8" });
            const url = URL.createObjectURL(blob);
            const link = document.createElement("a");
            link.href = url;
            link.download = fileName || "download";
...
window.easyLoginBase.preview = {
    base64: function (contentType, base64) {
        try {
            const cleanBase64 = (base64 || "").trim();
            const raw = window.atob(cleanBase64);
            const rawLength = raw.length;
            ...
            console.error("Failed to preview base64 file", ex);
        }
    }
};
...
window.easyLoginBase.share = {
    file: async function (fileName, contentType, base64, title, text) {
        try {
            if (!navigator.canShare || !navigator.share) {
                console.warn("Web Share API nao e suportada ou o contexto nao e seguro (HTTP).");
                return false;
            }
            ...
            console.error("Erro ao usar Web Share API:", ex);
        }
        return false;
    }
};
